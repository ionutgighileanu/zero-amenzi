...
import { FleetVehicleCard } from "@/components/app/FleetVehicleCard";
  ...
  return (
    <th className="px-2 text-left">
      <button
        ...
        className={`min-h-11 inline-flex items-center gap-1 px-2 text-[11px] font-bold uppercase tracking-wider hover:text-slate-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-700 rounded ${active ? "text-slate-900" : "text-slate-500"}`}
      >
        {label} <Icon size={12} />
      </button>
    </th>
...
export function FleetBoard({
  ...
  const driverRows = useMemo(() => {
    ...
    return drivers.filter(
      (d) => d.name.toLowerCase().includes(q) || d.phone.replace(/\s+/g, "").includes(digits)
    );
  }, [drivers, driverQuery]);
  ...
  const addDriverCert = async (id: string, cert: { type: string; expires: string }) => {
    try {
      ...
      setDrivers((list) =>
        list.map((d) =>
          d.id === id
            ? { ...d, certs: [...d.certs, { id: row.id, type: row.type, expires: row.expires_at }] }
            : d
        )
      );
    } catch (err) {
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <nav className="flex gap-6" aria-label="Secțiuni">
              {(
                ...
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`min-h-11 pb-3 sm:pb-0 inline-flex items-center gap-2 text-sm font-semibold border-b-2 sm:border-b-0 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-700 rounded-sm ${tab === id ? "text-slate-900 border-slate-900" : "text-slate-500 border-transparent hover:text-slate-700"}`}
                >
                  <Icon size={16} /> {label}
                  <span
          ...
          {tab === "vehicles" ? (
            ...
            >
              {/* Mobil/tabletă îngustă (< 768px): card-uri stivuite — decizie D-009 */}
              ...
              {/* ≥768px: tabelul original, neschimbat */}
